	...
	class WC_Customer_Order_Index {
		...
		public function update_index( $order_id ) {
			...
			$sql = $wpdb->prepare( 
				...
				$order_id, $user_id, $customer_email, $billing_email, $customer_name, $billing_name, $shipping_name, 
				$user_id, $customer_email, $billing_email, $customer_name, $billing_name, $shipping_name );
		...
		public function update_customer($user_id) {
			...
			$sql = $wpdb->prepare( 
				...
				$user->user_email, trim($user->first_name . " " . $user->last_name), $user_id
			); 
		...
		public function wc_customer_query_where($where, $wp_query) {
			...
			if (!empty($wp_query->query_vars['wc_customer_email'])) {
				$where .= $wpdb->prepare(" AND ( wc_cidx.customer_email = %s OR wc_cidx.billing_email = %s ) ", 
			...
			if (!empty($wp_query->query_vars['wc_customer_name'])) {
				...
				$where .= $wpdb->prepare(" AND ( wc_cidx.customer_name LIKE %s OR wc_cidx.billing_name LIKE %s OR wc_cidx.shipping_name LIKE %s ) ", 
					$name, 
					$name, 
					$name
				);
